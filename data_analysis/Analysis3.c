...
int main()
{
    ...
    */
    ...
    double VOL = 16*16*16*16; /*lattice spacing units */
    int step = 60;
    double t_0 = 4.8855; /*Sommer scale parameter, adimensional*/
    double sommer_scale_inverse = 1./t_0;
    double sigma_t_0 = 0.0015;

    double sigma_sommer_scale_inverse = err_prop_div(1.,t_0,0,sigma_t_0);

    printf("%lf %lf", sommer_scale_inverse, sigma_sommer_scale_inverse);

    int Wilson_flow_max = 600;
    int N_CNFG = 5004;
    int bin = 2;
    ...
    for(j=0; j<11; j++)
    {
        ...
        Q_SQUARE_MEAN_binned[j] = Q_square_mean_binned;
    ...
    */
    ...
    while(fraction <= 1.25 )
    {
        ...
        for(j=0; j<11; j++)
        {
            ...
            if(t_0*fraction*100 > Wilson_flow_max)
            {
                t_low = (double)((j-1)*step)/100.;
                t_up = (double)(j*step)/100.;
                index_up = j;
                index_low = j-1;
            }
